...
void	*runGibbs(void *arg){
	...
	double	*gammaDir = (*pargs) -> gammaDir;
	double	gammaSum = (*pargs) -> gammaSum;
						...
						if(matLoc != -1){
							for(occ = 0; occ < localDocMatrix[d][matLoc][1]; occ++){
								...
								for(k = 0; k < noTopics; k++){
									probDist[k] = (topicCountPerDoc[d][k] + alphaDir[k]) * (CwordBuffer[queueIndex * bundleSize + w][k] + gammaDir[word]) / (topicCountTotal[k] + gammaSum);
...
void	*fetchCtopicWorkload(void *arg){
	struct argStruct **pargs = (struct argStruct **)arg;
	int 	*stop = (*pargs) -> stop;
	int 	*topicCountTotal = (*pargs) -> topicCountTotal;
	int		noTopics = (*pargs) -> noTopics;
...
void	*fetchCwordWorkload(void *arg){
	struct argStruct **pargs = (struct argStruct **)arg;
	int 	*stop = (*pargs) -> stop;
	int		noTopics = (*pargs) -> noTopics;
	int		pdSize = (*pargs) -> pdSize;
...
void	*runGibbsWorkload(void *arg){
	struct argStruct **pargs = (struct argStruct **)arg;
	int 	*stop = (*pargs) -> stop;
	int 	*topicCountTotal = (*pargs) -> topicCountTotal;
	int		noTopics = (*pargs) -> noTopics;
	...
	while((*stop) == 0){
		counter ++;
		//if(counter % N == Z){printf("runGibbsWorkload looping\n");}
		// run Gibbs
		if(queueMat[queueIndex][0] == 1){
...
void	*updateCwordWorkload(void *arg){
	struct argStruct **pargs = (struct argStruct **)arg;
	int 	*stop = (*pargs) -> stop;
	int		noTopics = (*pargs) -> noTopics;
	int		pdSize = (*pargs) -> pdSize;
