...
void	createWindows(MPI_Win ***winArr, int pwSize, char procType, MPI_Comm *commArr, int prank, int CwordSize, int noTopics, int **wordCountPerTopic, int *localTopicCountTotal){
	...
	*winArr = malloc(pwSize * sizeof(MPI_Win*));
	...
	for(i = 0; i < pwSize; i++){
		if(procType == 'd'){
			MPI_Win_create(MPI_BOTTOM, 1, 1, MPI_INFO_NULL, commArr[i], &((*winArr)[i][0])); 
			MPI_Win_fence(0, (*winArr)[i][0]);
			MPI_Win_create(MPI_BOTTOM, 1, 1, MPI_INFO_NULL, commArr[i], &((*winArr)[i][1])); 
...
int 	main(int argc, char *argv[]){
	...
	sprintf(stopfile, "../../ChuckDesktop/Project/Inputs/stopwords.txt");
	int	printResults = 0;
	int 	maxFiles = 0;
	int	iterations = 100;
	...
	if(rank == 0){printf("CwordMethod:\t\t%d\n", CwordMethod);}
	if(rank == 0){printf("RMA:\t\t\t%d\n", rma);}
	if(rank == 0){printf("RMA for Gibbs:\t\t%d\n", gibbsRMA);}
