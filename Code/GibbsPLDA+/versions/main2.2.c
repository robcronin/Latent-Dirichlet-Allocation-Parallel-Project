...
void 	printHelp();
void 	parseArgs(int argc, char *argv[], int rank, MPI_Comm mainComm, char **directory, int *maxFiles, int *seed, int *noTopics, int *iterations, int *printResults, int *parallelReadIn, int *CwordMethod, int *rma, int *bundleSize, double *pwToPdRatio);
...
void    runPLDAplusGibbsPthreadsDeadline(char procType, int noTopics, int pdSize, int size, int bundleSize, int iterations, int totNoBundles, int circleStartBundle, int **bundles2d, int **bundles2dConv, struct treeRootInv *invertedIndexTree, int locNoFiles, int ***localDocMatrix, int ***topicIndex, int **topicCountPerDoc, double *alphaDir, double *betaDir, double betaSum, int **wordCountPerTopic, int *localTopicCountTotalAct, int rank, MPI_Comm pdComm, MPI_Comm mainComm){
	...
	if(procType == 'd'){
		...
		deadlines[0] = 25000 / sleepTime;
		deadlines[1] = 15000 / sleepTime;
		deadlines[2] = 10000 / sleepTime;
		

		int pwSize = size - pdSize; 		// FIXME
		int *workload;
		callocArrInt(&workload, pwSize);
		int overload = 1.2*(queueSize/pwSize);
...
int 	main(int argc, char *argv[]){
	...
	// PARSE ARGS
	parseArgs(argc, argv, rank, mainComm, &directory, &maxFiles, &seed, &noTopics, &iterations, &printResults, &parallelReadIn, &CwordMethod, &rma, &bundleSize, &pwToPdRatio);
	...
	// PRINT INFO
	printInfoLDA(rank, 1, LDAmethod, seed, stopfile, noTopics, noFiles, vocabSize, iterations, size, parallelReadIn, totalWords, pdSize, pwSize, CwordMethod, rma, threads, bundleSize);
	...
	// PRINT INFO
	printInfoLDA(rank, 2, LDAmethod, seed, stopfile, noTopics, noFiles, vocabSize, iterations, size, parallelReadIn, totalWords, pdSize, pwSize, CwordMethod, rma, threads, bundleSize);
...
MPI_Barrier(mainComm);
	...
	if(CwordMethod == 1){
		...
		if(threads == 1){
			...
			//runPLDAplusGibbsPthreadsBoth(procType, noTopics, pdSize, size, bundleSize, iterations, totNoBundles, circleStartBundle, bundles2d, bundles2dConv, invertedIndexTree, locNoFiles, localDocMatrix, topicIndex, topicCountPerDoc, alphaDir, betaDir, betaSum, wordCountPerTopic, localTopicCountTotal, rank, pdComm, mainComm);
			runPLDAplusGibbsPthreadsDeadline(procType, noTopics, pdSize, size, bundleSize, iterations, totNoBundles, circleStartBundle, bundles2d, bundles2dConv, invertedIndexTree, locNoFiles, localDocMatrix, topicIndex, topicCountPerDoc, alphaDir, betaDir, betaSum, wordCountPerTopic, localTopicCountTotal, rank, pdComm, mainComm);
		}
		else if(invMethod == 2){
			runPLDAplusGibbsMatrixInv(procType, noTopics, pdSize, size, bundleSize, iterations, totNoBundles, circleStartBundle, bundles2d, bundles2dConv, invertedIndexMatrix, locNoFiles, localDocMatrix, topicIndex, topicCountPerDoc, alphaDir, betaDir, betaSum, wordCountPerTopic, localTopicCountTotal, rank, wordLocationMat, vocabSize, mainComm);
		}
		else if(rma == 0 || rma == 1){
...
void printHelp(){
	...
	printf("-w [INT]\tSet RMA Method: 0=none, 1=init, 2=gibbs, 3=both (default: 0) Must use Cword Method 2\n");
...
void parseArgs(int argc, char *argv[], int rank, MPI_Comm mainComm, char **directory, int *maxFiles, int *seed, int *noTopics, int *iterations, int *printResults, int *parallelReadIn, int *CwordMethod, int *rma, int *bundleSize, double *pwToPdRatio){
	...
	if(rank == 0){
		int opt;
		while((opt=getopt(argc,argv,"a:b:c:d:hi:m:prs:t:w:"))!=-1){
			switch(opt){
				case 'a':
	...
	MPI_Bcast(pArgs, 9, MPI_INT, 0, mainComm);
